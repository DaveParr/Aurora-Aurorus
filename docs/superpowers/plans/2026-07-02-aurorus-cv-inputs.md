...
**Goal:** Wire all six of `aurorus`'s knob-controlled parameters (morph, rate, depth, feedback, mix, stereo width) to their matching CV inputs, using additive-offset-with-clamp — the same idiom the sibling `Aurora-Morse` project already uses for its Time CV.

**Architecture:** A new pure `Clamp01(float)` function in `aurorus/modulation.h` clamps the sum of a knob value and its CV value to `[0, 1]`. `aurorus/main.cpp`'s `AudioCallback` wraps all six `engine.SetX(hw.GetKnobValue(KNOB_X))` calls with `Clamp01(hw.GetKnobValue(KNOB_X) + hw.GetCvValue(CV_X))`, and the main LED loop's `blendColour(...)` call uses the same combined Warp value so the LED stays an accurate indicator of the actual (post-CV) morph position.
...
**Revision note (Task 3):** Tasks 1 and 2 below were written and executed under the original decision to leave `KNOB_WARP`/`CV_WARP` knob-only. After testing that version on real hardware, `CV_WARP` was expected to modulate morph like the other five CVs modulate their knobs. Task 3 reverses that decision — see `docs/superpowers/specs/2026-07-02-aurorus-cv-inputs-design.md`'s Revision note for the full rationale. Tasks 1 and 2's text below is preserved as an accurate historical record of what was built at the time; the Global Constraints above already reflect the current (post-Task-3) intent.
...
- `KNOB_WARP`/`CV_WARP` ARE wired (as of Task 3) — `engine.SetMorph(...)` and the LED loop's `blendColour(...)` both use `Clamp01(hw.GetKnobValue(KNOB_WARP) + hw.GetCvValue(CV_WARP))`. `GetWarpVoct()` remains unused — see the spec's Hardware Context for why the uncalibrated `GetCvValue(CV_WARP)` is used instead.
...
## Task 3: Wire Warp CV in too, and keep the LED in sync

After testing Tasks 1-2 on real hardware, `CV_WARP` was expected to modulate morph the same way the other five CVs modulate their knobs. This task reverses the original "Warp stays knob-only" decision — see `docs/superpowers/specs/2026-07-02-aurorus-cv-inputs-design.md`'s Revision note. It also updates the LED loop so the blend-colour indicator reflects the actual (post-CV) morph value, not just the knob — otherwise the LED would silently drift out of sync with what CV is doing to the audio.

This task has no host unit tests — same as Task 2, it's hardware wiring verified by a successful ARM build.

**Files:**
- Modify: `aurorus/main.cpp`

**Interfaces:**
- Consumes: `Clamp01(float) -> float` from Task 1 (already in `aurorus/modulation.h`, unchanged).

- [ ] **Step 1: Update `AudioCallback`'s `SetMorph` call**

The current line (`aurorus/main.cpp:27`):
...
Nothing else in `AudioCallback` changes — the other five `SetX` calls (added in Task 2) are unaffected.

- [ ] **Step 2: Update the LED loop's `blendColour` call**

The current line (`aurorus/main.cpp:57`):
...
This computes the same combined value as `AudioCallback`'s `SetMorph` call, independently, in the main loop — matching the existing architecture where the LED loop and the audio callback already independently call `hw.GetKnobValue(KNOB_WARP)` rather than sharing cached state across the audio/main-loop boundary.

- [ ] **Step 3: Update the top-of-file doc comment**

The current comment (`aurorus/main.cpp:1-12`):
...
- [ ] **Step 4: Rebuild the host tests to confirm no regression**

```bash
cd /home/dave/Development/Aurora-Aurorus/aurorus/tests && make
```

Expected: all existing test cases still pass unchanged (this task doesn't touch `modulation.h`/`.cpp` or any test file — `main.cpp` has no host-testable surface).

- [ ] **Step 5: Build the firmware to verify it compiles and links**
...
Expected: build succeeds, producing `aurorus/build/aurorus-dev.bin`. DTCMRAM usage should be unchanged (91.75%, 120,264 bytes) — this task adds no new persistent state.

- [ ] **Step 6: Update `README.md`**

The current CV Inputs section:
...
cd /home/dave/Development/Aurora-Aurorus
git add aurorus/main.cpp README.md
git commit -m "Wire Warp CV in too, keep LED in sync with post-CV morph value"
```
...
- **Spec coverage:** mapping table (Task 2 Step 1, Task 3 Step 1), behavior/clamp semantics (Task 1 + Task 2 Step 1), Warp wiring + LED sync (Task 3 Steps 1-2), doc comment updates (Task 2 Step 2, Task 3 Step 3), testing (Task 1), README updates (Task 2 Step 5, Task 3 Step 6) — every spec section (including the Revision) has a corresponding step.
- **Placeholder scan:** none found — every step has complete, exact code.
- **Type consistency:** `Clamp01(float) -> float` is declared identically in Task 1 (production code) and consumed identically in Task 2 and Task 3 (`main.cpp` call sites) — no signature drift.
